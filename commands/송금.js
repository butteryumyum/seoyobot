...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!user) return message.reply("**입금할 대상을 멘션해주세요**") // 멘션 한 유저가 없다면 리턴
        ...
        if (!sk) return message.reply("**가입을 먼저 진행해주세요. ( 서요야 지원금 )**") // 데이터베이스에서 userid 가 메세지를 보낸사람의 id인것이 없다면 리턴
        if (!tkdeoqkd) return message.reply("**입금할 사람이 없 서요**") // 위에꺼랑 비슷 (데이터베이스에서 멘션한 유저를 찾을 수 없으면 리턴)
        const betting = parseInt(args[1]) //2번쨰 args에서 숫자를 찾음
        if (!betting) return message.reply("**사용법 : 서요야 입금 @멘션 (금액)**") // 2번쨰 args에서 숫자를 못찾으면 리턴
        if (message.content.includes("-")) return message.reply("**금액엔 -가 포함되면 안돼요**") // 메세지 내용에 -가 포함되면 리턴
        if (betting < 500) return message.reply("**최소 입금 금액은 500원입니다**") // 2번쨰 args가 1000보다 작으면 리턴
        const money = parseInt(sk.money) // money를 메세지를 보낸사람의 돈으로 지정
        const money2 = parseInt(tkdeoqkd.money) // money2를 멘션한 유저의 돈으로 지정
        if (money < betting) return message.reply("**돈도 없는데 어따 보내는거야**")
        if (message.author.id == user.id) return message.reply("**자기자신을 사랑해도 돈은 못보내용!!**") // 맨션 한 유저가 메세지를 보낸유저와 아이디가 같다면 return
